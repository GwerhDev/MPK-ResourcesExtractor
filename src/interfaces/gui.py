...
def interface():
  ...
  if mpkinfo_file:
    ...
    if(mpkinfo_file["file"].lower().endswith(".mpkinfo")):
      ...
      sg.popup("Successful extraction", title="Success", background_color="#15171E", button_color='#0074e0', icon='../assets/logo.ico')

      # Opens "results" folder once finished
      subprocess.Popen(['explorer', os.path.abspath(mpkinfo_file["folder"] + "/MPL Resources Extractor")])
    else:
      sg.popup_error("Please select a valid MPKINFO file.", background_color="#15171E", button_color='#0074e0', icon='../assets/logo.ico')